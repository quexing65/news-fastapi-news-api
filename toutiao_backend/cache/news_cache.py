...
from config.cache_conf import get_json_cache, set_cache, delete_cache, delete_pattern_cache
...
# 写入新闻总数缓存
async def cache_news_count(category_id: int, count: int, expire: int = 1800) -> bool:
    key = f"{NEWS_COUNT_PREFIX}{category_id}"
...
# 浏览量更新后清理会受影响的缓存
async def invalidate_news_views_cache(news_id: int, category_id: int):
    await delete_cache(f"{NEWS_DETAIL_PREFIX}{news_id}")
    await delete_pattern_cache(f"{NEWS_LIST_PREFIX}{category_id}:*")
    await delete_pattern_cache(f"{RELATED_NEWS_PREFIX}*:{category_id}")
