...
async def create_user(db: AsyncSession, user_data: UserRequest):
    ...
    db.add(user)
    await db.commit()
    await db.refresh(user)      # 从数据库读回最新的 user 确保真的写入数据库
    return user


# 生成 Token
async def create_token(db:AsyncSession, user_id: User):
    # 生成 Token + 设置过期时间 -> 查询数据库当前用户是否有 Token -> 有：更行；没有：添加
    token = str(uuid.uuid4())
    # timedelta(days=7, hours=2, minutes=20, secends=10)
...
async def change_password(db: AsyncSession, user: User, old_password: str, new_password: str):
    ...
    # 更新：由SQLAlchemy真正接管这个 User 对象，确保可以 commit 提交
