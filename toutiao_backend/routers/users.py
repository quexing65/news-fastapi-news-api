...
async def register(user_data: UserRequest, db: AsyncSession = Depends(get_db)):# 用户信息 和 db
    ...
    #     }
    # }
    response_data = UserAuthResponse(token=token, user_info=UserInfoResponse.model_validate(user))  # type: ignore
...
async def login(
    ...
    user = await users.authenticate_user(
        db,
        user_data.username,
        user_data.password
    )
...
@router.get("/info")
async def get_user_info(user: User = Depends(get_current_user)) -> UserInfoResponse:
